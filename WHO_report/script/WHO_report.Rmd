...
# Draw plot and axes
plot('', xlim = range(xseq.f1), ylim = range(yseq.f1), xaxt = 'n', yaxt = 'n', xlab = '', ylab = '')
axis(1, at = xseq.f1, labels = format(xseq.f1, date.format), font = x.font.f1, cex = x.cex.f1)
axis(2, at = yseq.f1, font = y.font.f1, cex = y.cex.f1)

# Add axes labels
mtext('Cumulative number of cases', side = 2, font = ylab.f.f1, line = ylab.line.f1, cex = ylab.cex.f1)
mtext('Date (dd/mm)', side = 1, font = xlab.f.f1, line = xlab.line.f1, cex = xlab.cex.f1)

# Add grid lines
abline(v = xseq.f1, h = yseq.f1, col = 'grey', lty = 'dotted')

# Get order of countries by CumCases at present to create an temp copy of d, re-ordered. Allow to color the "top10" countries only.
tmp<- gather(tail(d,1)[-1], 'country', 'max') %>% arrange(-max)
d.tmp<- d[,c('date', rev(tmp$country))] 

# Loop through the data frame, add lines on by one, each with the corresponding color in palette.f1
for(c in 2:ncol(d.tmp)){
  lines(d.tmp[,c] ~ d.tmp$date, lwd = 2, col = palette.top10[c])
}
