...
```
**Figure 1. Epidemic curve for Scotland based on deaths over time up to `r format(today, "%d/%m/%Y")`.** Doubling time estimated over the past 7 days is `r Td.report[1,'Td.obs']` days (95%CI: `r Td.report[1,'ci.low']`-`r Td.report[1,'ci.upp']` days).
...
**Figure 3. Pairwise epidemic progression comparison**. The reported numbers are the numbers of days ahead (positive numbers, green) or behind (negative numbers, red) the regions in horizontal entries are relative to the regions in vertical entires.
...
**Figure 4. Comparison of epidemic curves for all Scottish Health Boards up to `r format(today, "%d/%m/%Y")`**. **A)** Cumulative reported cases. **B)** Cumulative cases per 10,000 population on log10 scale. Inset shows corresponding doubling times (in days) estimated over the past 7 days with 95% confidence intervals.
...
**Figure 6. Doubling time of cases**. Doubling times are calculated over a 7 day period up to `r format(today, "%d/%m/%Y")`. Error bars indicate 95%CI.
...
**Table 1. Summary of all doubling times and their 95%CI reported in the above figures.**.
