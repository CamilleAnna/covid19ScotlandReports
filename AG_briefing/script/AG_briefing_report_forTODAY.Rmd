...
* Across Health Boards in Scotland there is variation in cumulative case incidence (`r formatC(round(min(tail(df.fig3b, 1)[,-1]), 1), digit = 1, format = 'f')` to `r formatC(round(max(tail(df.fig3b, 1)[,-1]), 1), digit = 1, format = 'f')` per 10,000 population, Figures 4, 5) and doubling time (`r min(Td.report[5:nrow(Td.report),'Td.obs'])` to `r max(Td.report[5:nrow(Td.report),'Td.obs'])` days, Table 1).  
  
# Results
```{r fig1NEW, fig.height = 6, fig.width = 12, fig.fullwidth = TRUE}
d.uk.deaths<- 
