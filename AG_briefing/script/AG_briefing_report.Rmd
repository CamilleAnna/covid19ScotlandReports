...
**The epidemic in Scotland is ~`r abs(round(Scotland_vs_London, 0))` days `r text.Scotland_vs_London` London and is now growing at a XXXXXXX rate XXXXXX.**  
...
* The current 7-day doubling time for deaths in Scotland is `r Td.report[Td.report$variable == 'Scotland death','Td.obs']` days (95% confidence interval (CI): `r Td.report[Td.report$variable == 'Scotland death','ci.low']` - `r Td.report[Td.report$variable == 'Scotland death','ci.upp']` days) (Figure 1).  
...
* Across Health Boards in Scotland there is variation in cumulative case incidence (`r formatC(round(min(tail(df.fig3b, 1)[,-1]), 1), digit = 1, format = 'f')` to `r formatC(round(max(tail(df.fig3b, 1)[,-1]), 1), digit = 1, format = 'f')` per 10,000 population, Figures 4, 5) and doubling time (`r min(Td.report[5:nrow(Td.report),'Td.obs'])` to `r max(Td.report[5:nrow(Td.report),'Td.obs'])` days, Figure 7, Table 1).  
...
yseq = seq(0, round_any(max(d.death.sim.2$cumNumCases), 100, f = ceiling), yaxti.f1)
xseq = seq.Date(from = min(d.death.sim.2$date), to = max(d.death.sim.2$date), length.out = 5)
...
```
**Figure 1. Epidemic curve for Scotland based on deaths over time up to `r format(today, "%d/%m/%Y")`.** Doubling time estimated over the past 7 days is `r Td.report[Td.report$variable == 'Scotland death','Td.obs']` days (95% confidence interval (CI): `r Td.report[Td.report$variable == 'Scotland death','ci.low']`-`r Td.report[Td.report$variable == 'Scotland death','ci.upp']` days).
...
```{r fig7, echo = FALSE, fig.height = 8, fig.width = 7, out.width= "100%"}
